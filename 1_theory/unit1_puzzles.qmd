...
\frac{{43 \choose 20} \cdot 10^{23}}{10^{43}} = \frac{{43 \choose 20}}{10^{23}}.$$
...
Let $X = (X_1, ..., X_k)$ be a $\text{Multinomial}(n, \pi)$
...
On the other hand, the probability that all 10 kids 
receive 2+ cookies is $P(X_1 \geq 2)^{10} = (1-P(X_1 \leq 1))^{10} = 
(1-e^{-\frac{\lambda}{10}} - \frac{\lambda}{10}e^{-\frac{\lambda}{10}})^{10},$
since it's clear that $X_1, ..., X_{10}$ are independently and identically distributed. 
